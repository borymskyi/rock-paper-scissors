...
public class Bot {
    ...
    private void process(Update update) {
        ...
        if (message != null && message.viaBot() != null && message.viaBot().username().equals("pipkaGame0_bot")) {
            ...
            InlineQueryResultArticle pipka = buildInlineButton("pipka", "\uD83E\uDD1B Камень", "0");
            ...
            if (senderChose.equals(opponentChose)) {
                request = new EditMessageText(
                        chatId, messageId,
                        String.format(
                                "%s и %s выбрали %s. Опоненты выбрали одно и тоже. Ничья",
                                senderName,
                                opponentName,
                ...
                request = new EditMessageText(
                        chatId, messageId,
                        String.format(
                                "%s выиграл. %s выбрал %s и выиграл %s, а он выбрал %s",
                                senderName,
    ...
    private InlineQueryResultArticle buildInlineButton(String id, String title, String callbackData) {
        return new InlineQueryResultArticle(id, title, "Готов играть в камень, ножницы, бумага!")
                .replyMarkup(
                        new InlineKeyboardMarkup(
...
// проверка бота в телеге на уведомления и изминения в репе.
//